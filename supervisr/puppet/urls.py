...
urlpatterns = [
    ...
    url(r'^(?P<key>\w+)/v3/modules$', forge_api.module_list, name='module-list'),
    url(r'^(?P<key>\w+)/v3/modules/(?P<user>[a-z0-9]+)-(?P<module>[a-z0-9_]+)$',
        forge_api.module, name='module'),
    url(r'^(?P<key>\w+)/v3/users$', forge_api.user_list, name='user-list'),
    url(r'^(?P<key>\w+)/v3/users/(?P<user>[a-z0-9]+)$', forge_api.user, name='user'),
    url(r'^(?P<key>\w+)/v3/releases$', forge_api.release_list, name='release-list'),
    url(r'^(?P<key>\w+)/v3/releases/(?P<user>[a-z0-9]+)-'
        r'(?P<module>[a-z0-9_]+)-(?P<version>[a-z0-9\.\+]+)$',
        forge_api.release, name='release'),
    url(r'^(?P<key>\w+)/v3/files/(?P<user>[a-z0-9]+)-(?P<module>[a-z0-9_]+)-'
