...
def migrate_to_rs(apps, schema_editor):
    Record = apps.get_model('supervisr/dns', 'Record')
    ResourceSet = apps.get_model('supervisr/dns', 'ResourceSet')
    RecordResource = apps.get_model('supervisr/dns', 'RecordResource')
    UserProductRelationship = apps.get_model('supervisr/core', 'UserProductRelationship')
        ...
        for record in rec_v:
            ...
            for user in record.users.all():
                ...
                if not UserProductRelationship.objects.filter(product=rset, user=user).exists():
                    UserProductRelationship.objects.create(product=rset, user=user)

def connect_rs(apps, schema_editor):
    Zone = apps.get_model('supervisr/dns', 'Zone')
    Record = apps.get_model('supervisr/dns', 'Record')
    ResourceSet = apps.get_model('supervisr/dns', 'ResourceSet')
    UserProductRelationship = apps.get_model('supervisr/core', 'UserProductRelationship')
    ...
    for rec in Record.objects.all():
        ...
        rset = ResourceSet.objects.filter(name='%s, %s' % (rec.name, rec.domain.domain.name))
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ('supervisr/dns', '0003_auto_20170815_2125'),
    ]
    ...
    operations = [
        migrations.CreateModel(
            name='RecordResource',
            fields=[
                ('product_ptr', models.OneToOneField(auto_created=True, default=None, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='supervisr/core.Product')),
        ...
        migrations.CreateModel(
            ...
            fields=[
                ('product_ptr', models.OneToOneField(auto_created=True, default=None, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='supervisr/core.Product')),
                ('resource', models.ManyToManyField(to='supervisr/dns.RecordResource')),
            ],
            options={
                'abstract': False,
            },
        ...
        migrations.AddField(
            model_name='record',
            name='resource_set',
            field=models.ForeignKey(default=None, on_delete=django.db.models.deletion.CASCADE, to='supervisr/dns.ResourceSet'),
