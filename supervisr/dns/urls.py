...
urlpatterns = [
    url(r'^zones/$', zones.index, name='index'),
    ...
    url(r'^zones/(?P<zone>%s)/records/$' % DOMAIN_REGEX,
        records.list_records, name='record-list'),
    ...
    url(r'^zones/(?P<zone>%s)/records/(?P<record>.+)/(?P<uuid>%s)/update/$' %
        (DOMAIN_REGEX, UUID_REGEX), records.update, name='record-update'),
    url(r'^zones/(?P<zone>%s)/records/(?P<record>.+)/(?P<uuid>%s)/delete/$' %
        (DOMAIN_REGEX, UUID_REGEX), records.delete, name='record-delete'),
]
