...
def get_config_for_acs(acs_url):
    ...
    """
    for friendlyname, config in SAML2IDP_REMOTES.items(): # pylint: disable=unused-variable
...
def get_config_for_resource(resource_name):
    ...
    for friendlyname, config in SAML2IDP_REMOTES.items(): # pylint: disable=unused-variable
        ...
        for name, pattern in links: # pylint: disable=unused-variable
...
def get_deeplink_resources():
    ...
    """
    resources = []
    for key, sp_config in SAML2IDP_REMOTES.items(): # pylint: disable=unused-variable
        links = get_links(sp_config)
        for resource, patterns in links: # pylint: disable=unused-variable
            if '/' not in resource:
